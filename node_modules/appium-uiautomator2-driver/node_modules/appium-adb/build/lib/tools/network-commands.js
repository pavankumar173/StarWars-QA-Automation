...
async function getForwardList() {
    logger_1.log.debug(`List forwarding ports`);
    const connections = await this.adbExec(['forward', '--list']);
    return connections.split(node_os_1.EOL).filter((line) => Boolean(line.trim()));
}
/**
...
async function forwardPort(systemPort, devicePort) {
    logger_1.log.debug(`Forwarding system: ${systemPort} to device: ${devicePort}`);
...
async function removePortForward(systemPort) {
    logger_1.log.debug(`Removing forwarded port socket connection: ${systemPort} `);
...
async function getReverseList() {
    logger_1.log.debug(`List reverse forwarding ports`);
...
async function reversePort(devicePort, systemPort) {
    logger_1.log.debug(`Forwarding device: ${devicePort} to system: ${systemPort}`);
...
async function removePortReverse(devicePort) {
    logger_1.log.debug(`Removing reverse forwarded port socket connection: ${devicePort} `);
...
async function forwardAbstractPort(systemPort, devicePort) {
    logger_1.log.debug(`Forwarding system: ${systemPort} to abstract device: ${devicePort}`);
